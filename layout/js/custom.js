...
var smartClass = function(){
	...
	var fileInput = function(){
		...
		jQuery(document).on('change', '.btn-file :file', function() {
			...
			var	label = input.val().replace(/\\/g, 'https://smartclass.dexignzone.com/').replace(/.*\//, '');
		...
		jQuery(window).on('scroll', function () {
			if(jQuery('.sticky-header').length > 0){
				...
				if ($(window).scrollTop() > menu.offset().top) {
					...
					$('.header-smartClass .container > .logo-header .logo').attr('src','layout/images/logo.png');
				} else {
					menu.removeClass('is-fixed');
					$('.header-smartClass .container > .logo-header .logo').attr('src','error-404.php')
				}
			}
		});
